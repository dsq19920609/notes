...
/* eslint-disable global-require */  // 定义的所有eslint校验规则
const allRules = {
...
module.exports = {
  deprecatedRules, // 不推荐使用的规则
  rules: allRules, // 定义的所有规则
  configs: {
    recommended: { // 插件提供的recommended配置，可直接extends
      plugins: [
        'react'
      ],
      parserOptions: {
      ...
      rules: { // 开启的校验规则，这样就不需要自己一个一个的开启校验规则
